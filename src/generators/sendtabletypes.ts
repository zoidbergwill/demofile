...
import { PropType, SPROP_EXCLUDE, SPROP_INSIDEARRAY, SPROP_NOSCALE, SPROP_UNSIGNED } from "../props";
      ...
      for (const dt of demoFile.entities.dataTables) {
        ...
        for (const prop of dt.props) {
          ...
          if (prop.type == PropType.Int) {
            if (prop.numBits == 1) {
              typeStr = "boolean";
            } else if (prop.numBits == NUM_NETWORKED_EHANDLE_BITS && (prop.flags & SPROP_UNSIGNED) != 0 && (prop.flags & SPROP_NOSCALE) != 0) {
          ...
          if (name.charCodeAt(0) < 65 || name.indexOf(".") != -1 || name.indexOf("[") != -1) {
            name = `['${name}']`;
          }
