...
function parseDemoFile(path: string) {
  fs.readFile(path, function(err, buffer) {
    ...
    demoFile.gameEvents.on("player_death", (e) => {
      ...
      const victimColour = teamNumberToAnsi(victim ? victim.teamNumber : TeamNumber.Spectator);
      const victimName = victim ? victim.name : "unnamed";

      const attacker = demoFile.entities.getByUserId(e.attacker);
      const attackerColour = teamNumberToAnsi(attacker ? attacker.teamNumber : TeamNumber.Spectator);
