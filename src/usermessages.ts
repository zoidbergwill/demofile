...
export class UserMessages extends EventEmitter {
  ...
  public _handleUserMessage(msg: RequiredNonNullable<ICSVCMsg_UserMessage>) {
    let um = net.findUserMessageByType(msg.msgType);
    if (!um) {
      return;
    }

    if (this.listenerCount(um.name) || this.listenerCount("message")) {
      let msgInst = um.class.decode(msg.msgData);
