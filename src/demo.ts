...
export function parseHeader(buffer: Buffer): IDemoHeader {
  ...
  return {
    ...
    serverName: bytebuf.readString(MAX_OSPATH, ByteBuffer.METRICS_BYTES).split("\0", 2)[0],
    clientName: bytebuf.readString(MAX_OSPATH, ByteBuffer.METRICS_BYTES).split("\0", 2)[0],
    mapName: bytebuf.readString(MAX_OSPATH, ByteBuffer.METRICS_BYTES).split("\0", 2)[0],
    gameDirectory: bytebuf.readString(MAX_OSPATH, ByteBuffer.METRICS_BYTES).split("\0", 2)[0],
...
export class DemoFile extends EventEmitter {
  ...
  public _handleDataChunk() {
    readIBytes(this._bytebuf);
  }

  public _handleDataTables() {
    const chunk = readIBytes(this._bytebuf);
    this.entities._handleDataTables(chunk);
  }

  public _handleUserCmd() {
    this._bytebuf.readInt32(); // outgoing sequence
    this._handleDataChunk(); // TODO: parse user command
  }

  public _handleStringTables() {
    const chunk = readIBytes(this._bytebuf);
    const bitbuf = BitStream.from(chunk.buffer.slice(chunk.offset, chunk.limit));
